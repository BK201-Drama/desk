...
//! **为什么需要它**：变化来源全在 desk **外面**，没有监听时看板只是**冷启动那一刻的快照**。
//!
//! 推送分**两拍**（见 `rescan_then_icons`）：先让「多了一项」立刻可见，再把图标补上。
//!
//! ⚠️ **刻意不做 `meta::prune`**：本模块唯一能拿到的「还有哪些 key」来自 `scan_desktop()`，
//! 而它在 `read_dir` 失败时**静默返回空列表** —— 桌面一时读不到就把整份分类偏好清空（INV-4 禁止）。
...
/// 静默窗口：一次文件操作会接连放出好几个 raw event（Create / Modify / Attrib…），收拢到「安静了」再动手。
const QUIET: Duration = Duration::from_millis(250);

/// 静默窗口的封顶：持续写入（比如往桌面拷一个几 GB 的文件夹）会让静默窗口永远等不到，到点就扫一次。
const MAX_WAIT: Duration = Duration::from_secs(1);

/// 看板内容的**结构**指纹：`(围栏名, [项 id])`。⚠️ 只取结构、**不取图标** —— 图标是异步补的
/// 第二拍，拿它进指纹会让「图标刚到」和「桌面真的变了」分不开，第二拍就永远被自己拦掉。
...
/// 还没有图标缓存的桌面项 key。判据和 `index::ensure_icons` **完全一致**（就是「png 在不在」），免得两边打架。
...
/// 起监听。**失败不致命**（监听坏了只是退化成「只有冷启动那一次」），调用方拿到 `Err` 只该 `eprintln!`。
pub(crate) fn start(app: AppHandle) -> Result<(), String> {
    ...
    // watcher 一 drop 监听就停了，所以必须 move 进下面那个线程，和接收循环同生共死。
    ...
    for (origin, root) in &roots {
        ...
        let mut w = notify::recommended_watcher(move |res: notify::Result<notify::Event>| {
            ...
            // 看板是「重扫一遍」而不是「增量改」，所以只发一个「有动静」的信号，不搬运事件细节。
            let _ = tx.send(());
        })
    ...
    std::thread::spawn(move || {
        // 只为让 watcher 活到线程结束。⚠️ **不能写成 `let _ = watchers;`** —— 那会立刻 drop，监听当场停摆。
...
/// 收拢「安静 QUIET」或「总共等够 MAX_WAIT」之间的连串事件。返回即代表该动手了。
...
/// 第一拍：结构变了就推（方块暂时空的）；第二拍：图标补齐后再推一次（方块换成真图标）。
fn rescan_then_icons(app: &AppHandle, last: &mut Option<Fingerprint>, busy: &Arc<AtomicBool>) {
    ...
    // 已经有一个人在抽图标了就不叠线程 —— 叠起来会变成几十个 PowerShell 同时跑。漏不掉：
    // 在跑的那一轮收尾时会自己重新数一遍（见下面的循环）。**比 pending 标志结实**（标志会丢唤醒）。
    ...
    std::thread::spawn(move || {
        ...
        while !want.is_empty() {
            ...
            // 第二拍**刻意不看指纹**：按定义它就是「图标变了、结构没变」，拿指纹去挡它永远过不去。
...
mod tests {
    ...
    fn board(name: &str, items: Vec<FenceItemDto>) -> FenceDto {
        FenceDto {
            name: name.into(),
            items,
            // 显示偏好**刻意不进指纹**：收起/高度是纯前端偏好，改它不该触发重扫推送（前端自己就改了）。
    ...
    fn fingerprint_ignores_icon_arrival() {
        // 这正是第二拍的情形：id 一个没变，只有 `icon` 从 None 变成 Some —— 两者必须同指纹。
