// 「命令清单」的唯一解析器与渲染器：`build.rs` 与 crate 内的测试（`manifest_tests.rs`）
// 都用 `include!` 引入本文件，所以**必须**用普通注释 `//` 而不是 `//!` ——
// `include!` 把它拼进 crate 根的中段，那个位置不允许 `//!`。
//
// 命令名原先散在三处手写清单里（`lib.rs` 的 `generate_handler!` / `host/api.ts` 的
// `PERM_COMMANDS` / `e2e/tauri-mock.js` 的 `case`），漏一处只在**运行时**才炸。
// 现在注册表是唯一真相，另两处与它对齐。
//
...
///
/// **自校验**：块里每一条非空、非注释的行都必须产出合法标识符，产不出就 panic。
/// 刻意**不用**「数量 ≥ N」阈值 —— 解析被截断时它照样通过，正常删命令时又会误报。
...
pub fn render_ts(names: &[String]) -> String {
    ...
    s.push_str(
        "/**\n\
         \x20* ⚠️ **生成物 —— 不要手改**（手改会在下一次 cargo 构建时被原样覆盖）。\n\
         \x20* 由 `src-tauri/build.rs` 从 `src-tauri/src/lib.rs` 的 `generate_handler![...]` 解析而来，\n\
         \x20* 解析器在 `src-tauri/cmd_manifest.rs`；新鲜度由 `manifest_tests.rs` 的比对守。\n\
         \x20* 注册表是唯一真相，所以拼错命令名在这里是 `tsc` 错误，不是线上故障。\n\
         \x20*/\n\n",
    );
