...
- **壁纸着色**：命令面板「外观 → 壁纸着色」；开启后按当前墙纸主色给玻璃上淡 tint（与日/夜间正交）
- **天气**：Open-Meteo 一行简报，可改城市（默认关）
- **桌面整理**：桌面上的 Word/Excel/PPT 一键收进「桌面\整理\日期」（默认关；悬停可看清单）
...
| `%LOCALAPPDATA%\desk\plugins\<id>\` | 插件本地配置（如天气城市、整理脚本路径） |
